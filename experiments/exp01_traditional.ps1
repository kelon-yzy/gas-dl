...
try {
  ...
  python "$root\src\pipeline\train_traditional.py" --data-dir "$root\outputs\exp01_traditional" --output-root "$root\outputs\exp01_traditional" --tag "formal_seed42_core" --seed 42 --stage-filter stable --max-workers 1 --n-jobs -1 --n-perturbations 10 --xgb-n-jobs 4 --combo-list $coreCombos
  python "$root\src\pipeline\train_traditional.py" --data-dir "$root\outputs\exp01_traditional" --output-root "$root\outputs\exp01_traditional" --tag "formal_seed42_svr_diag" --seed 42 --stage-filter stable --train-limit 5000 --test-limit 1500 --max-workers 1 --n-jobs -1 --n-perturbations 10 --xgb-n-jobs 1 --combo-list $diagnosticCombos
