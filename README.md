...
> 创建日期：2026-05-14
> 状态：v0.3 文档同步阶段（已切换到 V3.1 双通道数据设计；代码与数据待第二步实现）
...
# 1. 公平 ML 基线
bash experiments/exp01_traditional.sh

# 2. DL 端到端基线
bash experiments/exp02_deep_e2e.sh

# 3. 融合策略对比
bash experiments/exp03_fusion_grid.sh

# 4. 留一域评估（G3 Track A）
bash experiments/exp04_domain_holdout.sh

# 5. 环境扰动（G3 Track B）
bash experiments/exp05_env_perturbation.sh

# 6. 跨域微调（G3 Track C，可选）
bash experiments/exp06_finetune_cross_domain.sh
