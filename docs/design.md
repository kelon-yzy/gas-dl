...
> 创建日期：2026-05-14（v0.3，已同步 V3.1 双通道数据设计）
...
| 实验组      | 模型                                   | 输入                                     | seed     | 输出目录             |
| -------- | ------------------------------------ | -------------------------------------- | -------- | ---------------- |
| 传统 ML    | Ridge                                | 双通道派生特征（TOF / tau / 峰值 / 反射峰统计 + slow） | 42/52/62 | exp01/ridge/     |
| 传统 ML    | PLS                                  | 同上                                     | 42/52/62 | exp01/pls/       |
| 传统 ML    | SVR (RBF)                            | 同上                                     | 42/52/62 | exp01/svr/       |
| 传统 ML    | XGBoost                              | 同上                                     | 42/52/62 | exp01/xgb/       |
| 传统融合     | SVR + Ridge dynamic fusion           | 同上                                     | 42/52/62 | exp01/svr_ridge/ |
| 传统融合     | SVR + PLS dynamic fusion             | 同上                                     | 42/52/62 | exp01/svr_pls/   |
| 传统融合     | XGB + Ridge dynamic fusion           | 同上                                     | 42/52/62 | exp01/xgb_ridge/ |
...
由 `src/sim/scripts/extract_dual_waveform_features.py` 从双通道波形重新导出，至少包含：

- 通道 1：matched filter TOF、主峰幅值、局部包络统计
- 通道 2：衰减时间常数 tau、反射峰间隔、反射峰计数、尾部能量统计
- slow 通道聚合统计

具体特征清单写到 `outputs/exp01_traditional/features/feature_manifest.json`。
...
| ML stacking 对照 | SVR + Ridge dynamic fusion                                                        | configs/traditional/svr_ridge.yaml |
...
| 单实验 stdout 格式 | `[exp_id.model.seedN] OK macro_RMSE=X.XXX took=Ys -> outputs/...` 或 `FAIL reason=... -> log path` |
...
├── train_traditional.py      # 包装 src/ml 训练
├── train_deep.py             # 包装 src/dl/training/train.py
...
| P1     | 传统 ML 基线（7 模型 × 3 seed） | 1-2 天 + CPU |
